...
- **Email cooldown:** 60 seconds between auth emails (signup, resend, forgot password). The API returns **429** if the user requests another message too soon. The verification **and** forgot-password pages disable Send/Resend for 60 seconds.
- Concurrent forgot/resend for the same account is serialized with a row lock; only one email is sent.
...
Forgot-password does not reactivate a locked account or skip email verification. The same **60 second email cooldown** applies as on signup/resend. The forgot-password screen counts down before Send/Resend can be used again.
